...
TEST(BitsetComplexTest, BitwiseOperations) {
    // Create test bitsets
    bitset a(3, 0);  // 3-bit bitset starting at offset 0
    bitset b(3, 0);
...
TEST(BitsetComplexTest, MixedTypeOperations) {
    ...
    // Test bitset | cube
    bitset result2 = a | test_cube;
    EXPECT_FALSE(result2.bits[0].is_null());
    EXPECT_FALSE(result2.bits[1].is_null());
    
    // Test bitset ^ cube
    bitset result3 = a ^ test_cube;
    
    // Test bitset & cover
    bitset result4 = a & test_cover;
...
TEST(BitsetComplexTest, DecompositionFunctions) {
    ...
    bitset decomposed = dut.decompose_hfactor(factors, 2, 0);
    ...
    bitset x_decomposed = dut.decompose_xfactor(xfactors, 2, 0);
